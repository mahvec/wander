...
export default function Contact() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('https://api.sheetapi.rest/api/v1/sheets/s5ijqaug8d27x1x0hll2ggwa', {
                method: 'POST',
                headers: {
                    ...
                    <div className="bg-white rounded-xl sm:rounded-2xl lg:rounded-3xl p-4 sm:p-6 lg:p-8 xl:p-10 shadow-xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                            ...
                            <button
                                ...
                                className={`w-full bg-[#1C77C3] hover:bg-linear-to-r from-[#1C77C3] to-[#F39237] text-white font-bold py-3 px-4 sm:py-4 sm:px-6 text-sm sm:text-base rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''
                                    }`}
                            >
                                {isSubmitting ? (
